...
任務：從這些新聞中，找出 3-5 個「值得做成深度專題報導」的潛藏角度。

判斷標準：
1. **跨新聞的連結** - 多則新聞串連後才能看出的模式、趨勢、矛盾
2. **被忽略的聲音** - 主流報導沒關注的群體、立場、影響
3. **系統性問題** - 個案背後的結構性原因（政策缺陷、制度盲點、權力關係）
4. **異常訊號** - 數據異常、言行矛盾、時間線可疑
5. **人物故事** - 有血有肉的當事人，可以深入追蹤的個案
...
{{
  "angles": [
    {{
      "title": "報導角度標題（15字內）",
      "description": "為何這是重要的深度報導角度（60字內）",
      "evidence": ["從新聞X發現...", "新聞Y和Z的矛盾是..."],
      "suggested_sources": ["建議採訪的對象/組織", "建議查閱的資料"],
      "priority": "high/medium"
    }}
  ],
  "summary": "這批新聞的整體觀察（100字內，聚焦在最值得挖掘的方向）"
}}"""
...
def init_scheduler():
    ...
    scheduler.add_job(update_international_news, 'cron', hour='*/2', minute='30')
    ...
    scheduler.start()
    print("[SCHEDULER] 排程已啟動 - 國內:每小時0分, 國際:每2小時30分, 摘要:08:00/12:00/18:00")
