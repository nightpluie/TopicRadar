...
### 1. 專案定位：深度報導與專題追蹤工具
- **目標**：為調查記者提供議題進展追蹤與靈感，非即時新聞監控系統
- **特色**：重視議題脈絡與趨勢分析，適合長期追蹤專題
- **更新頻率**：適中（每小時一次國內新聞、每小時一次國際新聞）
- **核心價值**：幫助記者發現新聞線索、追蹤議題進展、產生深度報導靈感

> **重要**：這不是即時新聞追蹤系統。設計哲學是「廣度收集、深度過濾」，讓記者專注在重要議題的長期進展。

### 2. 專業導向：介面規範
- **禁止使用顏文字（emoji）**：程式碼、UI、資料結構中一律不使用 emoji
- **純文字優先**：使用文字標籤、純文字描述，避免裝飾性符號
- **資訊密度**：專注於內容的可讀性與資訊豐富度
- **例外情況**：僅限 Markdown 文件的說明性標記（如本文件的說明符號）

### 3. 版本管理：語意化版本號
- **格式**：`主版本號.次版本號.修訂號`（Semantic Versioning）
- **當前版本**：v2.1.0
- **更新流程**：每次功能更新都必須更新 `CHANGELOG.md` 和前端版本顯示
